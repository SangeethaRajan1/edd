...
public class ValidLoginlogout_session2 extends Generictest {
	@Test(priority = 1,invocationCount=0)
	public void validloginlogout_session2() throws InterruptedException,
			IOException {

		// run in firefox
		String uname = ExcelGeneric.Excelread("ValidLogin", 2, 0);
		String pswd = ExcelGeneric.Excelread("ValidLogin", 2, 1);
		...
		App_config config = new App_config(driver);
		driver.manage().window().fullscreen();
		config.Clickappmmenu();
		...
		menu_rearrange.page_confirmation();
		test.pass("Confirmed the page");
		...
		test.pass("clicked on media library option");
		medialib.upload_logo();
		...
		test.pass("navigated to offline page");
		...
		test.pass("mouse hovered the app config tab");
		apptheme.apptheme_page();
		test.pass("navigated to app theme page");
		apptheme.select_theme();
		test.pass("save the BG color theme");
